...
export const cadastrarProfessor = async (req: Request, res: Response) => {
  ...
  try {
    const novoProfessor = await Professor.create({ nome, email, matricula, senha });
    res.status(201).json({
...
export const deletarProfessor = async (req: Request, res: Response) => {
  try {
    ...
    const disciplina = await Disciplina.findOne({ where: { professorId: professor?.id } });
...
export const listarProfessoresDeletados = async (req: Request, res: Response) => {
  try {
...
export const recuperarProfessor = async (req: Request, res: Response) => {
  try {
...
export const pegarProfessor = async (professorId: string) => {
  try {
    const professor = await Professor.findByPk(professorId, { paranoid: false });
